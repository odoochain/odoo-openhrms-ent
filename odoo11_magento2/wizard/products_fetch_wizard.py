...
class ProductsFetchWizard(models.Model):
    """Creates the model products fetch wizard"""
    ...
    fetch_type = fields.Selection([
        ...
        ('from_odoo', 'Export Products to Magento')
    ], string="Operation Type", default="to_odoo")
    ...
    def _import_products_list(self, config_products, existing_prod_ids,
                              template, attributes):
        ...
        cr.execute("select id, name from product_attribute where name is not null")
        all_attrib = cr.fetchall()
        odoo_attributes = {}
        for j in all_attrib:
            if j[1] and j[0]:
                for key in j[1].keys():
                    odoo_attributes[j[1][key]] = j[0]
        attributes_list = {}
        ...
        for j in all_attrib_vals:
            if j[1] and j[0]:
                for key in j[1].keys():
                    odoo_attribute_vals[j[1][key]] = j[0]
        for att in attributes['items']:
        ...
        for product in config_products:
            if str(product['id']) not in existing_prod_ids:
                ...
                if product_categ_ids:
                    ...
                    c_ids = cr.fetchall()
                template['name'] = json.dumps({str(language): product['name']})
                ...
                try:
                    for pic in product['media_gallery_entries']:
                        if 'thumbnail' in pic['types']:
                            image_file = product['media_gallery_entries'][0].get('file')
                    ...
                    logger.info("unable to import image url of product sku %s",
                                product['sku'])
                    pass
                if product.get('type_id') == 'configurable':
                    ...
                    if product.get('childs'):
                                ...
                                for att in child.get('custom_attributes'):
                                    ...
                                    if att['attribute_code'] == att_code:
                                        ...
                                        variant_categ_ids = attr.get(
                                            'value') or []
                            prod_id[tuple(variant_att_vals)] = {
                        ...
                        for i in product_product:
                            attr_id_list = []
                            for j in i.product_template_attribute_value_ids.product_attribute_value_id:
                                attr_id_list.append(j['id'])
                            if tuple(attr_id_list) in prod_id:
                                i.write(prod_id[tuple(attr_id_list)])
                else:
    ...
    def _update_custom_option(self, val, template, VariantObj, cr):
        """Method _update_custom_option to update the custom options of
        the product"""
        ...
        query_str = "insert into product_template (" + query_cols + \
                    ") values %s RETURNING id"
        cr.execute(query_str, (tuple(template.values()),))
    ...
    def _update_attributes(self, odoo_attributes, options, attributes):
        """Method _update_attributes to update the attributes of the product"""
        cr = self._cr
        ...
        for att in attributes:
            if str(att['attribute_id']) not in odoo_attributes and str(
                ...
                cr.execute(
                    "insert into product_attribute (name,create_variant,display_type,magento) "
                    " values(%s, 'always', 'radio', TRUE) returning id",
                    (att['attribute_code'],))
                rec_id = cr.fetchone()
    ...
    def get_product_data(self):
        """Method get_product_data to get the product data to export to
        magento"""
        catalog_data = []
        ...
        if products:
            for product in products:
                ...
                catalog_data.append(product_list)
    ...
    def fetch_products_to_odoo(self):
        """Method fetch_products_to_odoo to fetch the products from magento to
        odoo"""
        cr = self._cr
        # fetching products already fetched from magento to skip those already
        # created
        ...
        # in magento, each product can have more than one category, so we are
        ...
        categ_url = 'rest/V1/categories'
        type = 'GET'
        categ_list = self.env['magento.connector'].magento_api_call(headers={},
                                                                    url=categ_url,
                                                                    type=type)
        try:
        ...
        url = '/rest/V1/products?searchCriteria=0'
        type = 'GET'
        ...
        try:
            ...
            if simple_products_list:
                split_data = [simple_products_list[i:i + 10] for i in
                              range(0, len(simple_products_list), 10)]
                for data in split_data:
                    delay = self.with_delay(priority=1, eta=5)
                    delay._import_products_list(
                        data,
                        ids,
                        tmpl_vals,
                        attributes,
            ...
            # and their values
            if config_prod_list:
                split_data = [config_prod_list[i:i + 5] for i in
                              range(0, len(config_prod_list), 10)]
                for data in split_data:
                    delay = self.with_delay(priority=1, eta=5)
                    delay._import_products_list(
                        data,
            ...
            logger.info("Exception occured %s", e)
            raise exceptions.UserError(_("Error Occured %s") % e)
        logger.info("Products are successfully updated.")
        return {
    ...
    def fetch_products_from_odoo(self):
        """Method to fetch products from odoo to magento"""
        ...
        if not product_data:
            return
        split_data = [product_data[i:i + 5] for i in
                      range(0, len(product_data), 10)]
        for data in split_data:
            delay = self.with_delay(priority=1, eta=5)
            delay.export_product(product_datas=data, url=url,
                                 headers=headers, type=request_type)

    def export_product(self, product_datas, url, headers, type):
        """Method _export_product to export products to magento"""
        for data in product_datas:
            try:
                ...
                if product_list.get('sku'):
                    data[1].write({
                        'magento': True,
                        'magento_id': str(product_list.get('sku'))
                    })
                    data[1].product_tmpl_id.write({
                        'magento': True,
                        'magento_id': str(product_list.get('sku'))
                    })
            except Exception as e:
                pass
            logger.info(f"{data[1].name} product updated in Magento")

    def image_processing(self, file):
        """Method image_processing to process the image of the product"""
        if file:
            image = False
            magento_host = self.env['ir.config_parameter'].sudo().get_param(
                'odoo11_magento2.magento_host')
