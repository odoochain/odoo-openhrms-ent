...
class UpdateStockWizard(models.Model):
    """Creates the model update.stock.wizard"""
    _name = 'update.stock.wizard'
    _description = 'Update Stock Wizard'

    fetch_type = fields.Selection([
        ('to_odoo', 'Import stock status'),
        ('from_odoo', 'Export stock status')
    ], string="Operation Type", help="Type of fetching stock")

    def update_stock_item(self):
        """Method update_stock_item to update stock status"""
                ...
                if item.default_code:
                    ...
                    try:
                        if stock_item.get('extension_attributes') and \
                                stock_item['extension_attributes'].get(
                                    'stock_item'):
                            product_stock = stock_item['extension_attributes'][
                                'stock_item']
                            # updating qty on hand
                            inventory_wizard = UpdateQtyWiz.create({
                                'product_id': item.id,
                                'product_tmpl_id': item.product_tmpl_id.id,
                                'new_quantity': product_stock.get('qty'),
                            ...
                            if stock_item['price'] != item.list_price:
                                cr.execute(
                                    "update product_template set list_price=%s "
                                    "where id=%s",
                                    (stock_item['price'],
                                     item.product_tmpl_id.id))
                            logger.info("Successfully Updated %s",
                                        item.default_code)
                    except:
                        pass
            return {
    ...
    def _get_product_list(self, active_ids):
        """Method _get_product_list to retrieve product list"""
        sku_list = {}
